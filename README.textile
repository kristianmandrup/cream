...
The project has undergone a lot of bugfixes during 4-7 of May, thanks to @alecslupu who was patient enough to go through all the pain to find the bugs and assist me in fixing them. Thanks! Please use the latest master branches for cream, cancan-permits, roles_active_record, rails_assist and rails_artifactor. If you get problems from any other dependency gems, I recommend that you use their github master branch as the source.
I hope to release all gems ASAP when I know they all play nice ;)
...
Then think about which user types you need. User types are distinct types of users that might have individual data needs (separate table or schema), individual registration page or different administration rules (devise strategies such as email confirmation etc.)

Then decide which roles should be available for each user type, that is what kind of user_type each role makes sense for.

Example:

Roles: super admin, forum admin, guest user, normal user, content publisher

Would make sense to have these User types: User, Guest, Admin
...
This is done via a class method in the Roles API
